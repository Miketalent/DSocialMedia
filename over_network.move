...
module overmind::over_network {
    ...
    */
    entry fun like(
        ...
        let liker_pub_ref = borrow_global_mut<Publications>(liker_account_token_address);
        vector::push_back(&mut liker_pub_ref.likes, new_like);

        let pub_author_pub_ref = borrow_global_mut<Publications>(pub_author_account_token_address);
        if (publication_type == string::utf8(b"post")) {
            let post = vector::borrow_mut(&mut pub_author_pub_ref.posts, publication_id);
            vector::push_back(&mut post.likes, liker_username);
        } else {
            let comment = vector::borrow_mut(&mut pub_author_pub_ref.comments, publication_id);
            vector::push_back(&mut comment.likes, liker_username);
        };

        //emit a new like event
        let event = AccountLikeEvent {
            // The timestamp of when the account liked a publication
            timestamp: timestamp::now_seconds(),
            // The username of the account owns the publication
        ...
        let resource_address = get_resource_account_address();
        let events_store_ref = borrow_global_mut<ModuleEventStore>(resource_address);
        event::emit_event(&mut events_store_ref.account_like_events, event);
    }
    ...
    */
    entry fun unlike(
        ...
        assert_publication_type_to_like_valid(publication_type);
        assert_username_registered(unliker_username);
        assert_username_registered(publication_author_username);

        let unliker_account_token_address = get_account_token_address(unliker_username);
        let unliker_address = signer::address_of(unliker);
        assert_user_owns_account(unliker_address, unliker_account_token_address);  

        let pub_author_account_token_address = get_account_token_address(publication_author_username);
        assert_publication_exists(pub_author_account_token_address, publication_id, publication_type);

        assert_unliker_has_liked_publication(unliker_username, pub_author_account_token_address, publication_id, publication_type);

        //remove unliker's username from the publication's likes' list
        let pub_author_pub_ref = borrow_global_mut<Publications>(pub_author_account_token_address);
        if (publication_type == string::utf8(b"post")) {
            let post = vector::borrow_mut(&mut pub_author_pub_ref.posts, publication_id);
            ...
            let comment = vector::borrow_mut(&mut pub_author_pub_ref.comments, publication_id);
            vector::remove_value(&mut comment.likes, &unliker_username);
        };

        //remove this like from unliker's like list
        let liked_pub_reference = PublicationReference {
            // The address of the account that owns the publication
            publication_author_account_address: pub_author_account_token_address, 
            // The type of the publication
            publication_type: publication_type, 
        ...
        let unliker_pub_ref = borrow_global_mut<Publications>(unliker_account_token_address);
        let (_, liked_pub_index) = vector::find(&unliker_pub_ref.likes, |like| 
            {
                let like : Like = *like;
                like.reference == liked_pub_reference
            });
        vector::remove(&mut unliker_pub_ref.likes, liked_pub_index);

        //emit an unlike event
        let event = AccountUnlikeEvent {
            // The timestamp of when the account unliked a publication
            timestamp: timestamp::now_seconds(),
            // The username of the account owns the publication
            publication_owner_username: publication_author_username,
        ...
        let resource_address = get_resource_account_address();
        let events_store_ref = borrow_global_mut<ModuleEventStore>(resource_address);
        event::emit_event(&mut events_store_ref.account_unlike_events, event);
    }
    ...
    */
    ...
    /*
        Create and return account NFT address from the account username
    */
    inline fun create_account_token_address_with_username(username: String): address {
        let account_address = token::create_token_address(
    ...
    //==============================================================================================
    /*
        Looks up the account NFT address from given username in account registry
    */
    #[view]
    public fun get_account_token_address(username: String): address acquires State {
        let state_ref = borrow_global<State>(get_resource_account_address());
        *table::borrow<String,address>(&state_ref.account_registry.accounts, username)
    }
    ...
    */
    ...
    public fun get_global_timeline(
        ...
        let global_timeline_ref = borrow_global<GlobalTimeline>(get_resource_account_address());

        let all_post_author_metadata = vector::empty<AccountMetaData>();
        let all_posts = vector::empty<Post>();
        let has_liked_posts = vector::empty<bool>();

        let total_timeline_posts_count = vector::length(&global_timeline_ref.posts);
        let index_to_start = page_index * page_size;
        let count = 1;
        let timeline_copy = global_timeline_ref.posts;
        vector::reverse(&mut timeline_copy);

        while (count <= page_size && index_to_start < total_timeline_posts_count) {
            let ref = *vector::borrow(&timeline_copy, index_to_start);
            let post_author_address = ref.publication_author_account_address;
            let post_author_pub_ref = borrow_global<Publications>(post_author_address);

            vector::push_back(&mut all_post_author_metadata, *borrow_global<AccountMetaData>(post_author_address));

            let post = *vector::borrow(&post_author_pub_ref.posts, ref.publication_index);
            vector::push_back(&mut all_posts, post);
            vector::push_back(&mut has_liked_posts, vector::contains(&post.likes, &viewer_username));

            count = count + 1;
            index_to_start = index_to_start + 1;
        };

        (all_post_author_metadata, all_posts, has_liked_posts)
    }
    ...
    */
    ...
    public fun get_following_timeline(
        ...
        //make sure username is registered
        let state_ref = borrow_global<State>(get_resource_account_address());
        if (table::contains(&state_ref.account_registry.accounts, username)) {

            let global_timeline_ref = borrow_global<GlobalTimeline>(get_resource_account_address());
            let timeline_copy = global_timeline_ref.posts;
            vector::reverse(&mut timeline_copy);
            let account_token_address = get_account_token_address(username);
            let account_metadata = *borrow_global<AccountMetaData>(account_token_address);

            //get and store all following accounts address
            let following_account_addresses = vector::empty<address>();
            vector::for_each(account_metadata.following_account_usernames, |user| {
                let user: String = user;
                vector::push_back(&mut following_account_addresses, get_account_token_address(user));
            });
            
            let total_timeline_posts_count = vector::length(&global_timeline_ref.posts);
            let index_to_start = 0;
            let count = 1;
            let followed_post_count = 0;
            while (count <= page_size && !vector::is_empty(&following_account_addresses) && index_to_start < total_timeline_posts_count) {
                let ref = *vector::borrow(&timeline_copy, index_to_start);
                if (vector::contains(&following_account_addresses, &ref.publication_author_account_address)) {
                    
                    followed_post_count = followed_post_count + 1;
                    if(followed_post_count > page_index * page_size) {
                        vector::push_back(&mut all_post_author_metadata, *borrow_global<AccountMetaData>(ref.publication_author_account_address));
                        let author_pub_ref = borrow_global<Publications>(ref.publication_author_account_address);
                        let post = *vector::borrow(&author_pub_ref.posts, ref.publication_index);
                        vector::push_back(&mut all_posts, post);
                        vector::push_back(&mut has_liked_posts, vector::contains(&post.likes, &viewer_username));
                        count = count + 1;
                    };                    
    ...
    */
    ...
    public fun get_account_posts(
        ...
        if (table::contains(&state_ref.account_registry.accounts, username)) {
            let account_token_address = get_account_token_address(username);
            account_metadata = *borrow_global<AccountMetaData>(account_token_address);

            let account_pub_ref = borrow_global<Publications>(account_token_address);

            let posts_copy = account_pub_ref.posts;
            vector::reverse(&mut posts_copy);
            let post_count = 0;
            let count = 1;
            let index = 0;
            while (count <= page_size && index < vector::length(&posts_copy)) {       
                post_count = post_count + 1;
                if (post_count > page_index * page_size) {
                    let post = *vector::borrow(&posts_copy, index);
                    vector::push_back(&mut has_liked_posts, vector::contains(&post.likes, &viewer_username));
                    vector::push_back(&mut all_posts, post);
                    count = count + 1;
                };
        ...
        (account_metadata, all_posts, has_liked_posts)
    }   
    ...
    */
    ...
    public fun get_account_comments(
        ...
        let all_comment_posts = vector::empty<Post>();
        let all_post_author_metadata = vector::empty<AccountMetaData>();
        let all_comments = vector::empty<Comment>();
        let has_liked_comments = vector::empty<bool>();
        let has_liked_posts = vector::empty<bool>();
        let account_metadata = AccountMetaData {
            creation_timestamp: 0,
            account_address: @0x0, 
        ...
        if (table::contains(&state_ref.account_registry.accounts, username)) {
            ...
            let account_pub_ref = borrow_global<Publications>(account_token_address);
            let comments_copy = account_pub_ref.comments;
            vector::reverse(&mut comments_copy);
            let count = 1;
            let index = 0;
            let comment_count = 0;


            while (count <= page_size && index < vector::length(&comments_copy)) {

                comment_count = comment_count + 1;
                if (comment_count > page_size * page_index) {
                    let comment = *vector::borrow(&comments_copy, index);
                    vector::push_back(&mut all_comments, comment);
                    vector::push_back(&mut all_post_author_metadata, *borrow_global<AccountMetaData>(comment.reference.publication_author_account_address));
                    vector::push_back(&mut has_liked_comments, vector::contains(&comment.likes, &viewer_username));

                    let post_pub_ref = borrow_global<Publications>(comment.reference.publication_author_account_address);
                    let post = *vector::borrow(&post_pub_ref.posts, comment.reference.publication_index);
                    vector::push_back(&mut all_comment_posts, post);
                    vector::push_back(&mut has_liked_posts, vector::contains(&post.likes, &viewer_username));
        ...
        (account_metadata, all_comments, all_comment_posts, all_post_author_metadata, has_liked_comments, has_liked_posts)
    }
    ...
    */
    ...
    public fun get_account_liked_posts(
        ...
        let all_liked_posts = vector::empty<Post>();
        let all_post_author_metadata = vector::empty<AccountMetaData>();
        let has_liked_posts = vector::empty<bool>();
        let account_metadata = AccountMetaData {
            creation_timestamp: 0,
        ...
        if (table::contains(&state_ref.account_registry.accounts, username)) {
            ...
            let account_pub_ref = borrow_global<Publications>(account_token_address);
            let likes_copy = account_pub_ref.likes;
            vector::reverse(&mut likes_copy);
            let count = 1;
            let index = 0;
            let like_count = 0;

            while (count <= page_size && index < vector::length(&likes_copy)) {

                let like = *vector::borrow(&likes_copy, index);
                if (like.reference.publication_type == string::utf8(b"post")) {
                    like_count = like_count + 1;
                    if (like_count > page_size * page_index) {
                        let post_author_pub_ref = borrow_global<Publications>(like.reference.publication_author_account_address);
                        vector::push_back(&mut all_post_author_metadata, *borrow_global<AccountMetaData>(like.reference.publication_author_account_address));
                        let post = *vector::borrow(&post_author_pub_ref.posts, like.reference.publication_index);
                        vector::push_back(&mut all_liked_posts, post);
                        vector::push_back(&mut has_liked_posts, vector::contains(&post.likes, &viewer_username));
        ...
        (account_metadata, all_liked_posts, all_post_author_metadata, has_liked_posts)
    }
    ...
    */
    ...
    public fun get_account_liked_comments(
        ...
        let all_liked_comments = vector::empty<Comment>();
        let all_comment_author_metadata = vector::empty<AccountMetaData>();
        let all_comment_posts = vector::empty<Post>();
        let all_post_author_metadata = vector::empty<AccountMetaData>();
        let has_liked_comments = vector::empty<bool>();
        let has_liked_posts = vector::empty<bool>();
        ...
        if (table::contains(&state_ref.account_registry.accounts, username)) {
            ...
            while(count <= page_size && index < vector::length(&likes_copy)) {
                let like = *vector::borrow(&likes_copy, index);
                if (like.reference.publication_type == string::utf8(b"comment")) {
                    like_count = like_count + 1;
                    if (like_count > start_index) {
                        let commenter_pub_ref = borrow_global<Publications>(like.reference.publication_author_account_address);
                        vector::push_back(&mut all_comment_author_metadata, *borrow_global<AccountMetaData>(like.reference.publication_author_account_address));
                        let comment = *vector::borrow(&commenter_pub_ref.comments, like.reference.publication_index);
                        vector::push_back(&mut all_liked_comments, comment);
                        vector::push_back(&mut has_liked_comments, vector::contains(&comment.likes, &viewer_username));
        ...
        (account_metadata, all_liked_comments, all_comment_author_metadata, all_comment_posts, all_post_author_metadata, has_liked_comments, has_liked_posts)
    }
    ...
    */
    ...
    public fun get_account_followers(
        ...
        let all_metadata = vector::empty<AccountMetaData>();
        ...
        if (table::contains(&state_ref.account_registry.accounts, username)) {
            ...
            let followers_copy = account_metadata.follower_account_usernames;
            vector::reverse(&mut followers_copy);
            while(count <= page_size && index < vector::length(&followers_copy)) {
                metadata_count = metadata_count + 1;
                if (metadata_count > page_size * page_index) {
                    let user = *vector::borrow(&followers_copy, index);
                    let token_address = get_account_token_address(user);
                    vector::push_back(&mut all_metadata, *borrow_global<AccountMetaData>(token_address));
                    count = count + 1;
                };
                index = index + 1;
            };
    ...
    */
    ...
    public fun get_account_followings(
        ...
        if (table::contains(&state_ref.account_registry.accounts, username)) {
            ...
            let index = 0;
            let metadata_count = 0;
            let following_copy = account_metadata.following_account_usernames;
            vector::reverse(&mut following_copy);
            while(count <= page_size && index < vector::length(&following_copy)) {
                metadata_count = metadata_count + 1;
                if (metadata_count > page_size * page_index) {
                    let user = *vector::borrow(&following_copy, index);
                    let token_address = get_account_token_address(user);
                    vector::push_back(&mut all_metadata, *borrow_global<AccountMetaData>(token_address));
                    count = count + 1;
                };
    ...
    */
    ...
    public fun get_post(
        ...
        let post_author_metadata = AccountMetaData {
            creation_timestamp: 0,
            account_address: @0x0, 
            username: string::utf8(b""),
            name: string::utf8(b""),
        ...
        let state_ref = borrow_global<State>(get_resource_account_address());
        if (table::contains(&state_ref.account_registry.accounts, username)) {
            let post_author_account_address = get_account_token_address(username);
            let post_author_pub_ref = borrow_global<Publications>(post_author_account_address);

            //make sure post exists
            if (post_id < vector::length(&post_author_pub_ref.posts)) {
                post_author_metadata = *borrow_global<AccountMetaData>(post_author_account_address);
                post = *vector::borrow(&post_author_pub_ref.posts, post_id);
                has_liked = vector::contains(&post.likes, &viewer_username);

                let count = 1;
                let index = 0;
                let start_index = page_size * page_index;
                let comment_count = 0;
                while(count <= page_size && index < vector::length(&post.comments)) {
                    comment_count = comment_count + 1;
                    if (comment_count > start_index) {
                        let ref = *vector::borrow(&post.comments, index);
                        let commenter_pub_ref = borrow_global<Publications>(ref.publication_author_account_address);
                        vector::push_back(&mut all_comments, *vector::borrow(&commenter_pub_ref.comments, ref.publication_index));
                        vector::push_back(&mut all_metadata, *borrow_global<AccountMetaData>(ref.publication_author_account_address));
                        count = count + 1;
                    };
                    index = index + 1;
                };
        ...
        (post_author_metadata, post, has_liked, all_comments, all_metadata)
    }

    //==============================================================================================
    // Validation functions
    //==============================================================================================
    inline fun assert_content_length_ok(content: String, max: u64) {
        assert!(string::length(&content) <= max, EContentInvalidLength);
    }

    inline fun assert_bio_length_ok(bio: String, max: u64) {
        assert!(string::length(&bio) <= max, EBioInvalidLength);
    }

    inline fun assert_name_length_ok(name: String, max: u64) {
        assert!(string::length(&name) <= max, ENameInvalidLength);
    }

    inline fun assert_username_length_ok(username: String, max: u64) {
        let username_length = string::length(&username);
        assert!(username_length >= 1 && username_length <= max, EUsernameInvalidLength);
    }

    inline fun assert_profile_pic_uri_length_ok(uri: String, max: u64) {
        assert!(string::length(&uri) <= max, EProfilePictureUriInvalidLength);
    }

    inline fun assert_username_not_registered(username: String) acquires State {
        let state_ref = borrow_global<State>(get_resource_account_address());
        assert!(!table::contains(&state_ref.account_registry.accounts, username), EUsernameAlreadyRegistered);
    }

    inline fun assert_username_registered(username: String) acquires State {
        let state_ref = borrow_global<State>(get_resource_account_address());
        assert!(table::contains(&state_ref.account_registry.accounts, username), EUsernameNotRegistered);
    }

    inline fun assert_to_follow_username_list_ok(username: String, list: vector<String>, resource_address: address) acquires State {
        let state_ref = borrow_global<State>(resource_address);
        let dup = vector::empty<String>();

        vector::for_each(list, |name| {
            let name: String = name;

            assert!(!vector::contains(&dup, &name), EUserFollowsUser);
            vector::push_back(&mut dup, name);

            assert_follower_to_follow_name_different(username, name);
            assert!(table::contains(&state_ref.account_registry.accounts, name), EUsernameNotRegistered);
        });
    }

    inline fun assert_user_owns_account(user: address, account: address) {
        assert!(object::is_owner(object::address_to_object<token::Token>(account), user), EAccountDoesNotOwnUsername);        
    }

    inline fun assert_follower_to_follow_name_different(follower: String, to_follow: String) {
        assert!(follower != to_follow, EUsersAreTheSame);
    }

    inline fun assert_to_follow_not_already_followed(to_follow: String, list: vector<String>) {
        assert!(!vector::contains(&list, &to_follow), EUserFollowsUser);
    }

    inline fun assert_to_unfollow_was_followed(to_unfollow: String, list: vector<String>) {
        assert!(vector::contains(&list, &to_unfollow), EUserDoesNotFollowUser);
    }

    inline fun assert_publication_exists(account: address, id: u64, type: String) acquires Publications {
        let pub_ref = borrow_global<Publications>(account);

        if (type == string::utf8(b"post")) {
            assert!(id < vector::length(&pub_ref.posts), EPublicationDoesNotExistForUser); 
        };

        if (type == string::utf8(b"comment")) {
            assert!(id < vector::length(&pub_ref.comments), EPublicationDoesNotExistForUser);    
        };
    }

    inline fun assert_publication_type_to_like_valid(type: String) {
        assert!(type == string::utf8(b"post") || type == string::utf8(b"comment"), EPublicationTypeToLikeIsInvalid);
    }

    inline fun assert_publication_type_valid(type: String) {
        assert!(type == string::utf8(b"post") || type == string::utf8(b"comment"), EInvalidPublicationType);
    }

    inline fun assert_liker_not_already_liked_publication(
        liker_username: String,
        author_account: address, 
        id: u64,
        type: String) acquires Publications {

        let pub_ref = borrow_global<Publications>(author_account);

        if (type == string::utf8(b"post")) {
            let post = *vector::borrow(&pub_ref.posts, id);
            assert!(!vector::contains(&post.likes, &liker_username), EUserHasLikedPublication);
        } else {
            let comment = *vector::borrow(&pub_ref.comments, id);
            assert!(!vector::contains(&comment.likes, &liker_username), EUserHasLikedPublication);
        }; 
    }

    inline fun assert_unliker_has_liked_publication(
        unliker_username: String,
        author_account: address, 
        id: u64,
        type: String) acquires Publications {
        ...
        if (type == string::utf8(b"post")) {
            ...
            let comment = *vector::borrow(&pub_ref.comments, id);
            assert!(vector::contains(&comment.likes, &unliker_username), EUserHasNotLikedPublication);
        }; 
    }
